...
export default function SessionsPanel() {
    const { sessions, refresh, save, load, remove } = useSessionStore();
            ...
            {isOpen && (
                <div className="absolute right-0 top-full mt-2 z-50 w-80 rounded-lg border bg-card shadow-lg">
                    <div className="p-3 space-y-3">
                        ...
                        {!showSaveInput && tiles.length > 0 && (
                            <button
                                onClick={() => setShowSaveInput(true)}
                                className="w-full flex items-center justify-center gap-1.5 rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors"
                            >
                                <Save className="w-3.5 h-3.5" />
